...
<div on:click on:keydown tabindex="0" data-storage-file-id="{file.id ?? 'unknown'}" class="max-w-96 w-fit h-fit bg-surface-container hover:bg-surface cursor-pointer transition-colors rounded-2xl p-2 gap-2 flex" role="menuitem">
    <div class="w-fit overflow-hidden rounded-lg h-fit">
        <img class="w-fit max-h-20 object-cover" src="/images/file-fallback.png" alt="File preview fallback" />
    </div>

    <div class="w-fit flex min-w-40">
